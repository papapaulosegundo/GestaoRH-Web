...
class AuthPage extends Component {
  ...
  enviaCadastro = (e) => {
    ...
    if (cadSenha.length < 6)
      return toast.error('Senha deve ter mínimo 6 caracteres.')

    this.setState({ loading: true })

    api.post('/empresa/cadastrar', {
      cnpj: cadCnpj,
      razaoSocial: cadRazaoSocial,
      endereco: cadEndereco,
      telefone: cadTelefone,
      logoBase64,
      responsavelNome: cadNome,
      responsavelSobrenome: cadSobrenome,
      senha: cadSenha,
    })
      .then(() => {
      ...
      .catch(err => {
        toast.error(err.response?.data ?? 'Erro ao cadastrar empresa.')
      })
      .finally(() => {
        this.setState({ loading: false })
      })
    ...
    return (
      <Form onSubmit={this.enviaLogin}>
        <FormField label="CNPJ" className="form-group-rh">
          <InputWithIcon icon="bi-building">
            <Form.Control
        ...
        <FormField label="Senha" className="form-group-rh">
          <InputWithIcon icon="bi-lock" showToggle showPwd={showPwd}
